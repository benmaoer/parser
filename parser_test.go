...
func (s *testParserSuite) TestHintError(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(len(warns), Equals, 1)
	_, _, err = parser.Parse("/*+ zebra:w */ select 1 from (/*+ zebra:w */ select 1 from dual) as a", "", "")
	c.Assert(err, IsNil)
	c.Assert(len(warns), Equals, 1)
	_, _, err = parser.Parse("select 1 from dual /*+ zebra:w */", "", "")
	c.Assert(err, IsNil)
	c.Assert(len(warns), Equals, 1)
	_, _, err = parser.Parse("select 1 from (select 1 from dual /*+ zebra:w */) a /*+ zebra:w */", "", "")
	c.Assert(err, IsNil)
	c.Assert(len(warns), Equals, 1)
